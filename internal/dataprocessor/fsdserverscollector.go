...
func newFsdServersCollector(fsdServer *common.FSDServer) *FsdServersCollector {
	return &FsdServersCollector{
		Name: fsdServer.Name,
		CurrentUsers: prometheus.NewDesc("vatdns_dataprocessor_current_users",
			"Current amount of users connected server.",
			nil, prometheus.Labels{"server": fsdServer.Name},
		),
		MaxUsers: prometheus.NewDesc("vatdns_dataprocessor_max_users",
			"Maximum amount of users a server will allow at a time.",
			nil, prometheus.Labels{"server": fsdServer.Name},
		),
		AcceptingConnections: prometheus.NewDesc("vatdns_dataprocessor_accepting_connections",
			"If server is in rotation to be considered for connections",
			nil, prometheus.Labels{"server": fsdServer.Name},
		),
		RemainingSlots: prometheus.NewDesc("vatdns_dataprocessor_remaining_slots",
